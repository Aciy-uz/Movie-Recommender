...
// 电影映射数组 - 根据Movie 1, Movie 2等ID映射到真实电影名
const examples = [
  {
    "id": "1",
  ...
  {
    "id": "563",
    "title": "Stephen King's The Langoliers (1995)",
    "releaseDate": "01-Jan-1995"
  },
  {
    "id": "564",
  ...
  {
    "id": "600",
    "title": "Daniel Defoe's Robinson Crusoe (1996)",
    "releaseDate": "01-Jan-1996"
  },
  {
    "id": "601",
  ...
  {
    "id": "634",
    "title": "Microcosmos: Le peuple de l'herbe (1996)",
    "releaseDate": "11-Oct-1996"
  },
  {
    "id": "635",
  ...
  {
    "id": "1104",
    "title": "C'est arriv� pr�s de chez vous (1992)",
    "releaseDate": "01-Jan-1992"
  },
  {
    "id": "1105",
  ...
  },
  {
    ...
    "releaseDate": "16-Sep-1996"
  },
  {
    "id": "1359",
    "title": "Boys in Venice (1996)",
    "releaseDate": "24-Sep-1996"
  },
  {
    "id": "1360",
  ...
  {
    "id": "1420",
    "title": "Gilligan's Island: The Movie (1998)",
    "releaseDate": "01-Jan-1997"
  },
  {
    "id": "1421",
...
function getMovieTitle(movie) {
  // 如果API返回的title是"Movie 1"、"Movie 2"等格式，则从examples数组中获取真实电影名
  if (movie.title && movie.title.startsWith('Movie ')) {
    // 提取数字部分，如"Movie 1" -> 1
    const match = movie.title.match(/Movie (\d+)/);
    if (match) {
      const movieId = match[1];
      // 在examples数组中查找对应的电影
      const exampleMovie = examples.find(item => item.id === movieId);
      if (exampleMovie) {
        return exampleMovie.title;
      }
    }
  }
  // 否则使用API返回的标题
...
function getMovieYear(movie) {
  // 如果API返回的release_year不可用，尝试从examples数组中获取
  if (!movie.release_year || movie.release_year === '未知年份') {
    if (movie.title && movie.title.startsWith('Movie ')) {
      const match = movie.title.match(/Movie (\d+)/);
      if (match) {
        const movieId = match[1];
        const exampleMovie = examples.find(item => item.id === movieId);
        if (exampleMovie && exampleMovie.releaseDate) {
          // 从releaseDate中提取年份，如"01-Jan-1995" -> 1995
          const yearMatch = exampleMovie.releaseDate.match(/\d{4}/);
          if (yearMatch) {
            return yearMatch[0];
          }
        }
  ...
  return movie.release_year || '未知年份';
}

// 替换文本中的Movie X为真实电影名
function replaceMovieTitlesInText(text) {
  if (!text || typeof text !== 'string') return text;

  // 匹配《Movie 数字》或Movie 数字格式
  const moviePattern = /(《?)(Movie\s+(\d+))(》?)/g;

  return text.replace(moviePattern, (match, leftQuote, movieIdText, movieNum, rightQuote) => {
    // 查找对应的真实电影名
    const exampleMovie = examples.find(item => item.id === movieNum);
    if (exampleMovie) {
      // 保持原有的引号格式
      const leftQuoteChar = leftQuote || '';
      const rightQuoteChar = rightQuote || '';
      return `${leftQuoteChar}${exampleMovie.title}${rightQuoteChar}`;
    }
    // 如果找不到对应的电影，保持原样
    return match;
  });
...
// 从文本中移除Movie X格式的电影名
function removeMovieTitlesInText(text) {
  if (!text || typeof text !== 'string') return text;

  // 匹配《Movie 数字》或Movie 数字格式以及其后的分隔符
  // 匹配模式：电影名+可能的空格+竖线+空格
  const moviePattern = /(《?Movie\s+\d+》?\s*\|\s*)/g;

  // 移除匹配到的电影名和分隔符
  const result = text.replace(moviePattern, '');
...
async function getRecommendations() {
    const userDemand = searchInput.value.trim();
    if (!userDemand) {
        showError('请输入您的电影需求');
        return;
    }
    ...
    try {
        ...
        if (response.data.success) {
            displayRecommendations(response.data.data);
        } else {
            showError('推荐失败: ' + response.data.message);
        }
        ...
        if (axios.isCancel(error)) {
            ...
            // 服务器返回错误状态码
            showError(`服务器错误: ${error.response.status} - ${error.response.data.message || '未知错误'}`);
        } else if (error.request) {
            // 请求已发出但没有收到响应
            showError('网络错误，请检查API服务是否启动');
        } else {
            // 其他错误
...
function displayRecommendations(data) {
    ...
    if (recommendations.length === 0) {
        resultsTitle.textContent = '暂无推荐结果';
        resultsStats.textContent = '';
        movieGrid.innerHTML = '<p style="text-align: center; padding: 40px;">暂无推荐结果，请尝试其他需求描述</p>';
        resultsSection.style.display = 'block';
        return;
    }

    // 更新标题和统计信息
    resultsTitle.textContent = `推荐结果 (${recommendations.length}部电影)`;
    resultsStats.textContent = `处理时间: ${data.processing_time || 0}s | 请求ID: ${data.request_id || 'N/A'}`;
    ...
    recommendations.forEach(movie => {
        ...
        // 生成评分星星
        const rating = movie.predicted_rating || 0;
        ...
        // 生成推荐理由
        const reasonsHtml = generateReasonsHtml(movie.reasons || {});

        html += `
            <div class="movie-card">
                <div class="movie-rank">${movie.rank}</div>
                <div class="movie-title"><a href='https://www.sogou.com/web?query=${movieTitle}' style="color:#fff;"  target="_blank" >${movieTitle}</a></div>
                ${movie.original_title ? `<div class="movie-original-title">${movie.original_title}</div>` : ''}

                <div class="movie-meta">
                    <div class="movie-year">${movieYear}</div>
                    <div class="movie-rating">评分: ${rating.toFixed(1)} ${starsHtml}</div>
                    <div class="movie-match">匹配度: ${((movie.match_score || 0) * 100).toFixed(1)}%</div>
                    ${movie.duration ? `<div class="movie-duration">${movie.duration}分钟</div>` : ''}
                </div>
                ...
                <div class="movie-info-grid">
                    ${movie.directors && movie.directors.length ? `
                        <div class="info-item">
                            ...
                            <span class="info-value">${movie.directors.join(', ')}</span>
                        </div>
                    ` : ''}
                    ...
                    ${movie.actors && movie.actors.length ? `
                        <div class="info-item">
                            ...
                            <span class="info-value">${movie.actors.slice(0, 3).join(', ')}${movie.actors.length > 3 ? '...' : ''}</span>
                        </div>
                    ` : ''}

                    ${movie.country && movie.country.length ? `
                        <div class="info-item">
                                    <span class="info-label">国家:</span>
                                    <span class="info-value">${movie.country.join(', ')}</span>
                                </div>
                            ` : ''}
                ...
                ${movie.plot_summary ? `
                    <div class="movie-summary">
                        <strong>剧情简介:</strong> ${replaceMovieTitlesInText(movie.plot_summary)}
                    </div>
                ` : ''}

                <div class="movie-summary">
                    <strong>推荐摘要:</strong> ${replaceMovieTitlesInText(movie.detailed_summary || movie.quick_summary || '暂无简介')}
                </div>
                ...
                <div class="reasons-section">
                    <div class="reasons-title">🎯 详细推荐理由</div>
                    ${reasonsHtml}
                </div>
                ` : ''}
            </div>
...
// 生成推荐理由HTML
function generateReasonsHtml(reasons) {
    if (!reasons) return '';

    const reasonCategories = [
        { key: 'quick_summary', title: '简要推荐' },
        { key: 'detailed_summary', title: '详细描述' },
        { key: 'rating_analysis', title: '评分分析' },
        { key: 'genre_analysis', title: '类型分析' },
        { key: 'year_analysis', title: '年份分析' },
        { key: 'demand_match', title: '需求匹配' },
        { key: 'ensemble_analysis', title: '模型分析' },
        { key: 'viewing_advice', title: '观看建议' },
        { key: 'personalized_insights', title: '个性化洞察' },
        { key: 'comparative_analysis', title: '对比分析' },
        { key: 'key_highlights', title: '关键亮点' }
    ];

    let html = '';
    reasonCategories.forEach(category => {
        const content = reasons[category.key];
        if (content) {
            if (Array.isArray(content) && content.length > 0) {
                // 在推荐理由中移除电影名，而不是替换
                const processedContent = content.map(point => removeMovieTitlesInText(point));
                html += `
                    <div class="reason-category">
                        <div class="reason-category-title">${category.title}</div>
                        <div class="collapse-content collapsed">
                            ${processedContent.map(point => `<div class="reason-point">${point}</div>`).join('')}
                        </div>
                        <button class="collapse-btn" onclick="toggleCollapse(this)">展开</button>
                    </div>
                `;
            } else if (typeof content === 'string' && content.trim()) {
                // 在推荐理由中移除电影名，而不是替换
                const processedContent = removeMovieTitlesInText(content);
                html += `
                    <div class="reason-category">
                        <div class="reason-category-title">${category.title}</div>
                        <div class="collapse-content collapsed">
                            <div class="reason-point">${processedContent}</div>
                        </div>
                        <button class="collapse-btn" onclick="toggleCollapse(this)">展开</button>
                    </div>
                `;
...
function toggleCollapse(button) {
    ...
    button.textContent = content.classList.contains('collapsed') ? '展开' : '收起';
}
